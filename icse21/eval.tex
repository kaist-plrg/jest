...
\begin{itemize}
  ...
  \item {\bf RQ4 (Accuracy of Bug Localization)} The accuracy of bug locations
...
fails to a specific conformance test, we assume that the specification and
engines passing the test have bugs related to the test.  Otherwise, we assume
that engines failing the test have bugs.  We manually checked the detected bugs
whether they are actually engine bugs or specification bugs.  The following
...
\begin{table}[H]
  ...
  \[
    \begin{array}{l?r|r|r|r?r}
      ...
      \text{Engine Bugs}  & \inred{-} & \inred{-} & \inred{-} & \inred{-} & \inred{39}\\\hline
      \text{Spec. Bugs}   & \inred{-} & \inred{-} & \inred{-} & \inred{-} & \inred{7}
    \end{array}
  \]
  \vspace*{-1em}
\end{table}
...
asseritons.  The most reliable JavaScript engine is V8 because it contains only
two bugs and the worst one is Moddable XS with \inred{-} bugs.  For asseritons,
...
\textsf{Ord} are top-two to detect engine bugs and they detects \inred{-} out of
\inred{39} bugs
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}c@{~}?c|l|c|c|@{~}c@{~}|@{~}c@{~}|@{~}r@{}}
    ...
    ES11-1 &
    \text{Expression} &
    \makecell[l]{Unhandling abrupt completion in \textbf{Abstract Equality Comparison}} &
    \textsf{Exc} &
    O &
    2015-06-01 &
    2020-04-28 &
    ...
    ES11-2 &
    \text{Class} &
    \makecell[l]{Wrong order of properties for classes} &
    \textsf{Ord} &
    O &
    \inred{-} &
    \inred{-} &
    ...
    ES11-3 &
    \text{Function} &
    \makecell[l]{Missing property \code{name} for functions} &
    \textsf{Ord} &
    O &
    \inred{-} &
    \inred{-} &
    ...
    ES11-4 &
    \text{Expression} &
    \makecell[l]{Using the wrong variable \code{oldvalue} instead of \code{oldValue} in\\
      \textbf{Evaluation} of \textit{UpdateExpression}} &
    \textsf{-} &
    O &
    \inred{-} &
    \inred{-} &
    ...
    ES11-6 &
    \text{Class} &
    \makecell[l]{Missing property \code{name} for classes} &
    \textsf{Ord} &
    O &
    \inred{-} &
    \inred{-} &
...
\begin{figure}[t]
  ...
  \caption{Ranks of actual bug locations in results of \textsf{Bug Localizer}
    for all detected engine or specification bugs by $\tool$}
  \label{fig:localize}
  \vspace*{-1em}
\end{figure}
