...
import kr.ac.kaist.ires.error.UnexpectedShift
import kr.ac.kaist.ires.parser.TokenParsers
...
trait AlgoCompilerHelper extends GeneralAlgoCompilerHelper {
  ...
  override lazy val etcStmt: P[Inst] = "change its bound value to" ~> id ^^ {
    ...
    } | "Let" ~> id <~ "be this production" ^^ {
      case x => ILet(Id(x), toERef("this"))
    } | ("If the code matched by this production is strict mode code , let" ~> id <~ "be") ~ valueParser <~ rest ^^ {
      case x ~ y => ILet(Id(x), y)
    } | "If Type (" ~ id ~ ") is Reference , IsPropertyReference (" ~ id ~ ") is" ~ value ~ ", and GetReferencedName (" ~ id ~ ") is" ~ code ~ rest ^^^ {
      emptyInst
  ...
  override lazy val etcExpr: P[I[Expr]] = (
    ("the String value of length 1 , containing one code unit from" ~> id) ~ (", " ~> ("namely" | "specifically") ~> "the code unit at index" ~> id) ^^ {
      ...
      case x => {
        val tempP = getTemp
        pair(parseInst(s"""{
          access $tempP = ($x "Evaluation")
        }""") :: Nil, ERef(RefId(Id(tempP))))
      }
    } | (id <~ ". ChainEvaluation (") ~ (id <~ ",") ~ (id <~ ") .") ^^ {
      case x ~ y ~ z => {
        val tempP = getTemp
        val tempP2 = getTemp
        pair(parseInst(s"""{
          access $tempP = ($x "ChainEvaluation")
          app $tempP2 = ($tempP $y $z)
        }""") :: Nil, ERef(RefId(Id(tempP2))))
      }
    } | ("this OptionalChain .") ^^^ {
      pair(Nil, ERef(RefId(Id("OptionalChain"))))
