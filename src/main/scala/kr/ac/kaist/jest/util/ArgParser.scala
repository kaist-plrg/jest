...
import kr.ac.kaist.jest.{ JEST, Command, JESTConfig }
import kr.ac.kaist.jest.phase.{ PhaseOption, Config }
import kr.ac.kaist.jest.error._
import spray.json._

// Argument parser by using Scala RegexParsers.
class ArgParser(cmd: Command, jestConfig: JESTConfig) extends RegexParsers {
  var ruleList: List[Parser[Unit]] = Nil

  var optNameSet: Set[String] = Set()

  addRule(jestConfig, "", JEST.options)
  ...
  def apply(args: List[String]): Unit = {
        ...
        Source.fromFile(fileName, "UTF-8").mkString.parseJson match {
          case (obj: JsObject) => obj.fields.foreach {
            case (phase, value: JsObject) => {
              if (JEST.phases.map(_.name).contains(phase))
    ...
    lazy val fileName: Parser[Unit] = str ^^ {
      s => jestConfig.fileNames = s :: jestConfig.fileNames
    }
    ...
    jestConfig.fileNames = jestConfig.fileNames.reverse
  }
}
