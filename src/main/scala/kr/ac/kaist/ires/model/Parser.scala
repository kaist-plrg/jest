...
object Parser extends ESParsers {
  ...
  lazy val IterationStatement: ESParser[IterationStatement] = memo(args => {
    ...
    log((
      log((((((MATCH <~ t("do")) ~ Statement(List(pYield, pAwait, pReturn, false)) <~ t("while")) <~ t("(")) ~ Expression(List(true, pYield, pAwait)) <~ t(")")) <~ t(";")) ^^ { case _ ~ x0 ~ x1 => IterationStatement0(x0, x1, args) })("IterationStatement0") |
