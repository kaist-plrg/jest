  ...
  def addBuiltin(
    ...
    case (m, (givenName, length, func)) =>
      ...
      (m.get(baseAddr) match {
        case Some(IRMap(ty, map)) => m ++ List(
          baseAddr -> IRMap(ty, map + (propV -> descAddr)),
          descAddr -> m.getOrElse(descAddr, IRMap(Ty("PropertyDescriptor"), Map(
            Str("Value") -> addr,
        ...
        case _ =>
          addr -> IRMap(Ty("BuiltinFunctionObject"), BuiltinFunctionObject.map - Str("Construct") ++ Map(
            Str("Code") -> func,
        ...
        NamedAddr(s"$name.SubMap") -> (m.getOrElse(NamedAddr(s"$name.SubMap"), IRMap(Ty("SubMap"), Map())) match {
          case IRMap(ty, map) => IRMap(ty, map ++ List(
        ...
        NamedAddr(s"DESC:$name.name") -> m.getOrElse(NamedAddr(s"DESC:$name.name"), IRMap(Ty("PropertyDescriptor"), Map(
          Str("Value") -> Str(propName),
          Str("Writable") -> Bool(false),
          Str("Enumerable") -> Bool(false),
          Str("Configurable") -> Bool(true)
        ))),
        NamedAddr(s"DESC:$name.length") -> m.getOrElse(NamedAddr(s"DESC:$name.length"), IRMap(Ty("PropertyDescriptor"), Map(
