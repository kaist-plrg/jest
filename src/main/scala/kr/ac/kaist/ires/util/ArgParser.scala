...
import kr.ac.kaist.ires.{ IRES, Command, IRESConfig }
import kr.ac.kaist.ires.phase.{ PhaseOption, Config }
import kr.ac.kaist.ires.error._
import spray.json._

// Argument parser by using Scala RegexParsers.
class ArgParser(cmd: Command, iresConfig: IRESConfig) extends RegexParsers {
  var ruleList: List[Parser[Unit]] = Nil

  var optNameSet: Set[String] = Set()

  addRule(iresConfig, "", IRES.options)
